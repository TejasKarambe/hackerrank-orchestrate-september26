...
def add_days(d_str, days):
    return format_date(parse_date(d_str) + timedelta(days=days))
...
def generate_cashflows_v3(user_id, request_date, msg_adjustments, spending_changes=None):
    ...
    if salary_info and salary_info["amount"] > 0:
        ...
        while cur_dt <= end_dt:
            ...
            pay_day = min(salary_info["pay_day"], max_d)
    ...
    # Group by category for variable categories, and by (category, description) for fixed categories
    variable_cats = {"dining", "groceries", "transport", "shopping", "entertainment"}
    groups = defaultdict(list)
    for e in past_evs:
        if e["direction"] == "debit" and e["category"] != "salary" and e["status"] == "settled":
            key = (e["category"], e["category"]) if e["category"] in variable_cats else (e["category"], e["description"])
            groups[key].append(e)
    ...
    for (cat, desc), ev_list in groups.items():
        ...
        if cadence >= 25:
            ...
            while cur_dt <= end_dt:
                ...
                exp_date = cur_dt.replace(day=p_day)
...
# Test on 25 samples
print("Testing full sample suite with v3 engine:")
for s in samples:
    u_id = s["user_id"]
    r_date = s["request_date"]
    p = profiles[u_id]
    r_amt = s["requested_amount"]
    gt_safe = s["gt_amount_safe_to_pay"]
    
    msg_adj = analyze_messages_for_user(u_id, messages_by_user.get(u_id, []), r_date)
    cfs = generate_cashflows_v3(u_id, r_date, msg_adj)
    lowest, _ = simulate_balance(p["current_available_balance"], p["minimum_balance_to_keep"], cfs, [])
    max_safe = lowest - p["minimum_balance_to_keep"]
    safe = max(0.0, min(r_amt, max_safe))

    diff = safe - gt_safe
    status = "OK" if abs(diff) < 1.0 else f"DIFF: {diff:+.2f}"
    if status != "OK":
        print(f"  {s['request_id']} ({u_id}): safe={safe:.2f}, GT={gt_safe:.2f} [{status}]")
    else:
        print(f"  {s['request_id']} ({u_id}): safe={safe:.2f} [PASS]")
